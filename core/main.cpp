...
static int coverage(int argc, char** argv) {
    ...
    CascadeCfg cfg = CascadeCfg::make(size);
    // Dense boundary-jitter sweep: ξ ∈ {0, 1/8, …, 1} covers the full
    // 4^{ξ−1/2} split range INCLUDING the near-paraxial-breakdown band
    // (ε ∈ [0.8, 1)) that a coarse {min, 1, max} sweep skips — a finite
    // enumeration must at least sample the regime the lemma says is
    // hardest.
    double jf[9];
    for (int i = 0; i < 9; i++) jf[i] = std::pow(4.0, i / 8.0 - 0.5);
